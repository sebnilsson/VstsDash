    ...
    internal static class TeamBoardScoresHelper
    {
            ...
            int maxCount)
        {
            ...
            foreach (var candidateGroup in candidateGroups)
            {
                ...
                {
                    var earnedAt = candidateGroup.Select(x => GetPointEarnedAt(x.WorkItem))
                        .OrderByDescending(x => x)
                        .FirstOrDefault();

                    foreach (var item in candidateGroup)
                        yield return (item.Key, new Point(
                            TeamMemberPointType.Assist,
                            value,
                            id,
                            description,
                            earnedAt));

                    if (totalTopItems == maxCount) yield break;
                }
                else
                {
            ...
            IReadOnlyCollection<TeamMemberApiResponse> teamMembers)
        {
            ...
            foreach (var workItem in doneWorkItems)
            {
                var doneChildItems = workItem.ChildItems
                    .Where(
                        x => x.IsStateDone && x.IterationPath == iteration.Path
